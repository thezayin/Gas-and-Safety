...
/**
 * Executes the use case to fetch a specific user profile.
 *
 * The parameters required to identify the profile, encapsulated in [GetProfileByIdParams].
 * @return A [Flow] emitting a [Resource] containing the [ProfileModel] on success
 *         or an error message on failure.
 */
interface GetProfileByIdUseCase : suspend (GetProfileByIdParams) -> Flow<Resource<ProfileModel>>
...
class GetProfileByIdUseCaseImpl(
     ...
     *         or an error message on failure.
     */
    override suspend fun invoke(params: GetProfileByIdParams): Flow<Resource<ProfileModel>> =
        flow {
            ...
            try {
                ...
                emit(Resource.Error("Failed to retrieve profile with ID ${params.id}: ${e.localizedMessage}"))
            }
